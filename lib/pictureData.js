...
class MusicInfoLoader {
  constructor(fileUri, options) {
    ...
    this.options = options
      ? {
          ...
          // duration: options.duration ? options.duration : false,
          // filename: options.filename ? options.filename : false,
    ...
    // if (this.options.title == true) this.expectedFramesNumber++;
    // if (this.options.artist == true) this.expectedFramesNumber++;
    // if (this.options.album == true) this.expectedFramesNumber++;
    // if (this.options.genre == true) this.expectedFramesNumber++;
    // if (this.options.duration == true) this.expectedFramesNumber++;
    // if (this.options.filename == true) this.expectedFramesNumber++;
    if (this.options.picture == true) this.expectedFramesNumber++;
  ...
  async processFrame() {
    ...
    if (frameID === EMPTY) this.finished = true;
    else {
      chunk = await this.read(4);
  ...
  async processPictureFrame(frameSize) {
    ...
    this.frames[PICTURE_TOKEN] = {
      ...
      pictureData:
        "data:" + mimeType + ";base64," + this.bytesToBase64(pictureData),
    };
  }
